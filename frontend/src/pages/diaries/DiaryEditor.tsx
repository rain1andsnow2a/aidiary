...
import {
  PenLine, Calendar, MessageCircle, Star, Smile, CloudSun, AlertCircle, Trophy, HeartHandshake,
  HelpCircle, Sparkles, Battery, Heart, Angry, Frown, PartyPopper, ChevronRight, RefreshCw,
} from 'lucide-react'
...
export default function DiaryEditor() {
  ...
  useEffect(() => {
    ...
    const draft: DiaryDraft = {
      title, content, contentHtml, diaryDate, emotionTags, importanceScore,
      savedAt: Date.now(),
    }
  ...
  const toggleEmotionTag = (tag: string) => {
    setEmotionTags((prev) => (prev.includes(tag) ? prev.filter((x) => x !== tag) : [...prev, tag]))
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(158deg, #f8f5ef 0%, #f2eef5 58%, #f5f2ee 100%)' }}>
      <header className="sticky top-0 z-50 backdrop-blur-xl border-b border-stone-200/70 relative" style={{ background: 'rgba(248,245,239,0.88)' }}>
        <div className="absolute top-2 right-4"><LanguageSwitcher /></div>
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex justify-between items-center py-3.5">
            ...
            <span className="text-sm font-semibold text-stone-600 flex items-center gap-1.5">
              ...
              {isEditMode ? t('diary.editDiary') : t('diary.newDiary')}
            </span>
            <button
          ...
          <div className="card-warm p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-stone-300 mt-1">
                {guidanceSource === 'ai' ? t('editor.aiGuidance') : t('editor.fallbackGuidance')}
              </p>
            </div>
            <button
              type="button"
            ...
            <div className="flex flex-wrap gap-2">
              {PRESET_EMOTIONS.map(({ label, icon, key }) => (
                <button
                  ...
                  style={emotionTags.includes(key) ? { background: 'linear-gradient(135deg, #e88f7b, #a09ab8)' } : undefined}
                >
                  {icon}
                  {label}
                </button>
